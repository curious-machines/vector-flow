...
/// Marker stored in temp data to request a fit-all on the next frame.
#[derive(Clone, Copy)]
struct FitAllRequest;
...
impl SnarlState {
    pub fn load<T>(
        ...
        // Check for a fit-all request.
        let fit_all = cx.data_mut(|d| {
            let key = id.with("fit_all");
            let has = d.get_temp::<FitAllRequest>(key).is_some();
            if has {
                d.remove::<FitAllRequest>(key);
            }
            has
        });
...
impl<T> Snarl<T> {
    ...
    /// Replace the set of selected nodes in the UI.
    ///
    /// Use `id_salt` as well as [`Id`] and [`Context`] of the [`Ui`] that were used in [`Snarl::show`] method.
    pub fn set_selected_nodes_at(
        ...
        cx.data_mut(|d| {
            if nodes.is_empty() {
                d.remove::<SelectedNodes>(snarl_id);
            } else {
    ...
    /// Request that the next frame fits all nodes into view.
    ///
    /// Uses the same `id_salt` and `id` as [`Snarl::show`].
    pub fn request_fit_all(id_salt: impl Hash, id: Id, cx: &Context) {
        let snarl_id = id.with(id_salt);
        cx.data_mut(|d| d.insert_temp(snarl_id.with("fit_all"), FitAllRequest));
    }
}

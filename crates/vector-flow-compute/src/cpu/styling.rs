...
mod tests {
    ...
    fn set_stroke_on_shape() {
        ...
        let color_data = NodeData::Color(Color::WHITE);
        let result = set_stroke(&data, &color_data, 3.0, LineCap::Butt, LineJoin::Miter(4.0), vec![], 0.0, 0.0);
        if let NodeData::Shape(s) = result {
    ...
    fn set_stroke_on_shapes_batch() {
        ...
        let data = NodeData::Shapes(Arc::new(shapes));
        let color_data = NodeData::Color(Color::WHITE);
        let result = set_stroke(&data, &color_data, 2.0, LineCap::Butt, LineJoin::Miter(4.0), vec![], 0.0, 0.0);
        if let NodeData::Shapes(s) = result {
            assert_eq!(s.len(), 1);
    ...
    fn set_stroke_with_colors_batch() {
        let data = NodeData::Shapes(Arc::new(make_shapes(3)));
        let colors = NodeData::Colors(Arc::new(vec![RED, GREEN, BLUE]));
        let result = set_stroke(&data, &colors, 2.0, LineCap::Butt, LineJoin::Miter(4.0), vec![], 0.0, 0.0);
        if let NodeData::Shapes(s) = result {
            assert_eq!(s.len(), 3);
